...
class ewdupcpCustomPostTypes {
	...
	public function __construct() {
		...
		// Sort categories table by custom ordering, save updated order
		add_action( 'pre_get_posts', array( $this, 'sort_categories_by_order' ) );
		add_action( 'wp_ajax_ewd_upcp_update_category_order', array( $this, 'update_category_order' ) );
	...
	public function load_cpts() {
		...
		$args = array(
			...
			'rewrite' 		=> array( 
				'slug' 			=> $ewd_upcp_controller->settings->get_setting( 'permalink-base' ) 
			),
			'supports' 		=> array(
				'title', 
				'editor',
	...
	public function show_product_details( $post ) { 
		...
		<input type="hidden" name="ewd_upcp_nonce" value="<?php echo $this->nonce; ?>">
			...
			<div class='ewd-upcp-meta-field'>
				...
				<div class='ewd-upcp-meta-field-input'>
					...
					<?php if ( $custom_field->type == 'textarea' ) { ?>
							...
							<textarea id='ewd-upcp-<?php echo esc_attr( $custom_field->name ); ?>' name='ewd_upcp_custom_field_<?php echo esc_attr( $custom_field->id ); ?>'>
					...
					<div class='ewd-upcp-related-product'>
						...
						<select name='related_products[]'>
							...
							<option value=''></option>
								...
								<?php foreach ( $products as $product ) { ?>
									<option value='<?php echo $product->ID; ?>' <?php echo ( $related_product == $product->ID ? 'selected' : '' ); ?>><?php echo esc_html( $product->post_title ); ?></option>
								<?php } ?>
			...
			<div class='ewd-upcp-meta-field-input'>
				<select name='previous_product'>
					<option value=''></option>
						...
						<?php foreach ( $products as $product ) { ?>
							<option value='<?php echo $product->ID; ?>' <?php echo ( $previous_product == $product->ID ? 'selected' : '' ); ?>><?php echo esc_html( $product->post_title ); ?></option>
						<?php } ?>
			...
			<div class='ewd-upcp-meta-field-input'>
				<select name='next_product'>
					<option value=''></option>
						...
						<?php foreach ( $products as $product ) { ?>
							<option value='<?php echo $product->ID; ?>' <?php echo ( $next_product == $product->ID ? 'selected' : '' ); ?>><?php echo esc_html( $product->post_title ); ?></option>
						<?php } ?>
			...
			<div class='ewd-upcp-meta-field-input'>
				...
				<?php foreach ( $product_videos as $product_video ) { ?>
					...
					<div class='ewd-upcp-product-video'>
						...
						<div class='ewd-upcp-product-image-image'>
							<img src='<?php echo esc_attr( $product_video->url ); ?>' />
						</div>
			...
			<div class='ewd-upcp-catalog-meta-add-items'>
				...
				<div class='ewd-upcp-catalog-meta-add-items-selection ewd-upcp-hidden' data-selected='categories'>
					...
					<?php foreach ( $categories as $category ) { ?>
						<label class='ewd-upcp-admin-input-container'>
							<input type='checkbox' class='ewd-upcp-add-items-category-checkbox' value='<?php echo $category->term_id; ?>' />
						...
						<?php foreach ( $items as $item ) { ?>
							...
							<tr>
								...
								<td>
									...
									<input type='hidden' name='catalog_item_id[]' value='<?php echo $item->id; ?>' />

									<input type='hidden' name='catalog_item_type[]' value='<?php echo $item->type; ?>' />
	...
	public function save_product_meta( $post_id ) {
		...
		if ( ! empty( $_POST['product_video_url'] ) ) {
	...
	public function save_product_videos( $post_id ) {
		...
		foreach ( $_POST['product_video_url'] as $key => $video_url ) {
	 ...
	 * @since 3.0.0
	 */
	public function sort_categories_by_order( $query ) {

		if ( ! is_admin() ) { return; }

		if ( ! isset( $_GET['taxonomy'] ) ) { return; } 

		if ( $_GET['taxonomy'] != EWD_UPCP_PRODUCT_CATEGORY_TAXONOMY and $_GET['taxonomy'] != EWD_UPCP_PRODUCT_TAG_TAXONOMY ) { return; }
	...
	public function update_category_order() {
		...
		foreach ( $ids as $order => $term_id ) {
			...
			update_term_meta( intval( $term_id ), 'order', $order );
		}
	}
	...
	public function update_category_data( $term_id ) { 
		...
		if ( empty( get_term_meta( $term_id, 'order', true ) ) ) {
			...
			update_term_meta( $term_id, 'order', 999 );
		}
	}
	...
	public function register_product_table_columns( $defaults ) {
		...
		$defaults['ewd_upcp_duplicate'] = __( 'Post ID', 'ultimate-product-catalogue' );
	...
	public function display_product_columns_content ( $column_name, $post_id ) {
		...
		if ( $column_name == 'ewd_upcp_duplicate' ) {

			echo '<a href="' . add_query_arg( 'duplicate_product', $post_id ) . '">' . __( 'Duplicate', 'ultimate-product-catalogue' ) . '</a>';
		}
	}
