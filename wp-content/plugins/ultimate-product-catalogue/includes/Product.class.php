...
class ewdupcpProduct {
	...
	public function set_filtering_price() {
		...
		$this->filtering_price = ( empty( $this->current_price ) ? 0 : preg_replace( '~\D~', '', $this->current_price ) ) / $divisor;
	}
	...
	public function get_display_price() {
		...
		return $ewd_upcp_controller->settings->get_setting( 'currency-symbol-location' ) == 'before' ? $ewd_upcp_controller->settings->get_setting( 'currency-symbol' ) . $this->current_price : $this->current_price . $ewd_upcp_controller->settings->get_setting( 'currency-symbol' );
	}
	...
	public function get_image() {
		...
		return '<img class="ewd-upcp-product-image" src="' . $image_url . '" />';
	}
