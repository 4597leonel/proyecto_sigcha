...
class ewdupcpBackwardsCompatibility {
	...
	public function run_backwards_compat() {
		...
		if ( ! taxonomy_exists( EWD_UPCP_PRODUCT_CATEGORY_TAXONOMY ) ) {
			...
			set_transient( 'ewd-upcp-run-backwards-compat', true, 30 );
		...
		set_transient( 'ewd-upcp-backwards-compat-running', true, 30 );
		...
		$settings = array(
			...
			'show-catalog-information'					=> get_option( 'UPCP_Show_Catalogue_Information' ) == 'NameDescription' ? array( 'name', 'description' ) : strtolower( get_option( 'UPCP_Show_Catalogue_Information' ) ),
	...
	public function convert_products() {
		...
		$old_products = $wpdb->get_results( "SELECT * FROM $products_table_name" );
		...
		foreach ( $old_products as $old_product ) {
			...
			if ( $post_id ) {
				...
				// Custom Fields
				$uploads_dir = site_url( '/wp-content/uploads/upcp-product-file-uploads/' );
