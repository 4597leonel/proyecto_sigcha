...
class ewdupcpDashboard {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_ewd_upcp_hide_upgrade_box', array($this, 'hide_upgrade_box') );
		add_action( 'wp_ajax_ewd_upcp_display_upgrade_box', array($this, 'display_upgrade_box') );
	...
	public function enqueue_scripts() {
		...
		if ( $currentScreen->id == 'upcp_product_page_ewd-upcp-settings' ) {
								...
								<?php
									if ( empty( $catalogs ) ) {echo "<tr><td colspan='3'>" . __("No catalogs to display yet. Create a catalog for it to be displayed here.", 'ultimate-product-catalogue') . "</td></tr>";}
										...
										foreach ( $catalogs as $catalog ) { ?>
											<tr>
												<td><a href='edit.php?post=<?php echo $catalog->ID; ?>&action=edit'><?php echo $catalog->post_title; ?></a></td>
												<td>[product-catalogue id='<?php echo $catalog->ID; ?>']</td>
												<td><?php echo get_post_meta( $catalog->ID, 'product_count', true ); ?></td>
											</tr>
										<?php }
									}
								?>
