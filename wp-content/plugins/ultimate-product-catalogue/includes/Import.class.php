...
class ewdupcpImport {
	...
	public function import_products() {
	    ...
	    foreach ( $data as $product_data ) {
	        ...
	        foreach ( $product_data as $col_index => $value ) {
	            ...
	            if ( $col_index == $name_column ) { $post_data['post_title'] = sanitize_text_field( $value ); }
            	elseif ( $col_index == $slug_column ) { $post_data['post_name'] = sanitize_text_field( $value ); }
            	elseif ( $col_index == $description_column ) { $post_data['post_content'] = $value; }
            	...
            	elseif ( $col_index == $catalog_id_column ) { $post_meta['catalog_id'] = intval( $value ); }
            		...
            		foreach ( $custom_fields as $custom_field ) {
            			...
            			if ( $col_index == $custom_field->column ) { $product_custom_fields[ $custom_field->id ] = sanitize_text_field( $value ); }
            		}
            	}
	        }
	        ...
	        if ( $post_id ) {
	        	...
	        	if ( ! empty( $post_meta['category'] ) or ! empty( $post_meta['subcategory'] ) ) { 
	        		...
	        		$categories = ! empty( $post_meta['subcategory'] ) ? explode( ',', $post_meta['category'] ) : array();
	...
	public function enqueue_import_scripts() {
		...
		if ( $screen->id == 'ewd-upcp-tracking_page_ewd-upcp-import' ) {

			wp_enqueue_style( 'ewd-upcp-admin-css', EWD_UPCP_PLUGIN_URL . '/assets/css/ewd-upcp-admin.css', array(), '5.0.0' );
			wp_enqueue_script( 'ewd-upcp-admin-js', EWD_UPCP_PLUGIN_URL . '/assets/js/ewd-upcp-admin.js', array( 'jquery' ), '5.0.0', true );
		}
	}
