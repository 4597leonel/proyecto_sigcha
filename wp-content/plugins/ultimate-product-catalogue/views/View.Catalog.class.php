...
class ewdupcpViewCatalog extends ewdupcpView {
	...
	public function maybe_print_cart_form() {
		...
		if ( empty( $ewd_upcp_controller->settings->get_setting( 'product-inquiry-cart' ) ) and empty( $ewd_upcp_controller->settings->get_setting( 'woocommerce-checkout' ) ) ) { return; }
	...
	public function print_sidebar_custom_fields() {
		...
		foreach ( $custom_fields as $custom_field ) {
			...
			if ( empty( $this->sidebar_custom_fields[ $custom_field->id ] ) ) { return; }
	...
	public function get_cart_action_url() {
		...
		return ! empty( $ewd_upcp_controller->settings->get_setting( 'woocommerce-checkout' ) ) ? get_permalink( $woocommerce_page_id ) : get_permalink();
	}
	...
	public function get_cart_submit_label() {
		...
		return empty( $ewd_upcp_controller->settings->get_setting( 'woocommerce-checkout' ) ) ? $this->get_label( 'label-send-inquiry' ) : $this->get_label( 'label-checkout' );
	}
		...
		foreach ( $product_posts as $product_post ) {
			...
			if ( ! empty( $this->filtering_custom_fields ) ) { 
				...
				foreach ( $this->filtering_custom_fields as $field_id => $field_values ) {
					...
					if ( $control_types[ $field_id ] == 'slider' and ( $product->custom_fields[ $field_id ] < min( $field_values ) or $product->custom_fields[ $field_id ] > max( $field_values ) ) ) { continue 2; }				
					elseif ( empty( array_intersect( (array) $product->custom_fields[ $field_id ], $field_values ) ) ) { continue 2; }
				}
			}
	...
	public function enqueue_assets() {
		...
		wp_localize_script( 'ewd-upcp-js', 'ewd_upcp_php_data', $data );
